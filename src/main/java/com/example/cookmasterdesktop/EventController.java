...
public class EventController {
    ...
    String[] eventTypes = {"Dégustation", "Cours sur site", "Cours à domicile"};
    @FXML
    void fetch(ActionEvent event)
    {
        ...
        series.setName("Nombre d'events par mois");
        ...
        series2.setName("Types d'événements");
        for (int i = 0; i < eventTypes.length; i++)
        {
            series2.getData().add(new XYChart.Data<>(eventTypes[i], Math.random() * 1000));
        }
        //Générer des données aléatoire pour le graphique en ligne
        LineChart.getData().clear();
        XYChart.Series<String, Double> series3 = new XYChart.Series<>();
        LineChart.setLegendVisible(false);
        series3.setName("Fréquence de planification");
        for (int i = 0; i < month.length; i++)
        {
        ...
        PieChart.setTitle("Events les plus réservés");
        for(int i = 0; i < eventTypes.length; i++)
        {
            PieChart.getData().add(new PieChart.Data(eventTypes[i], Math.random() * 1000));
        }
